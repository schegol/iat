...
<?if($arItems):?>
	...
	<?endif;?>
			...
			foreach($arItems as $i => $arItem):?>
				...
				?>
				<a class="services-list__wrapper services-list__wrapper--new <?=$itemWrapperClasses?>" href="<?=$detailUrl?>">
					<div class="services-list__item <?=$itemClasses?> <?=($bDetailLink ? '' : 'services-list__item--cursor-initial')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
						<?if($bShowImage && $imageSrc):?>
							<div class="services-list__item-image-wrapper <?=$imageWrapperClasses?>">
								...
								<?if(
									...
									<?if($bDetailLink):?>
										<div class="arrow-all stroke-theme-target">
						...
						<?if($bFonImg):?>
							<?if($bDetailLink):?>
								<div class="services-list__item-link services-list__item-link--absolute" href="<?=$detailUrl?>"></div>
							<?endif;?>
						...
						<div class="services-list__item-text-wrapper services-list__item-text-wrapper--new flexbox <?=($bShowBottom ? 'services-list__item-text-wrapper--has-bottom-part'.(($arParams['IMAGE_POSITION'] === 'TOP' || $bTRLImg) ? ' flexbox--justify-beetwen' : '') : '')?>">
							<?if($bFonImg):?>
								<?if($bDetailLink):?>
									<span class="services-list__item-link services-list__item-link--absolute" href="<?=$detailUrl?>"></span>
								<?endif;?>
							<?endif;?>
							...
							<div class="services-list__item-text-top-part <?=($bFonImg ? 'srollbar-custom scroll-deferred' : '')?> <?=($bSRLImg && !$arParams['NARROW'] && !($bShowImage && $imageSrc)) ? 'flex-1' : ''?>">
								...
								<div class="services-list__item-title switcher-title font_<?=$arParams['NAME_SIZE']?>">
									<?if($bDetailLink):?>
										<span class="dark_link color-theme-target" href="<?=$detailUrl?>"><?=$arItem['NAME']?></span>
									<?else:?>
									...
									<?elseif($arParams['IMAGE_POSITION'] === 'TOP_LEFT'):?>
										<?if($bDetailLink):?>
											<?if($arParams['ELEMENTS_ROW'] == 1):?>
												<div class="arrow-all arrow-all--wide stroke-theme-target">

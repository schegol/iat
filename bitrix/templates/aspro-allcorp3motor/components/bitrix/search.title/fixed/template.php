					...
					<form action="<?=$arResult["FORM_ACTION"]?>" class="search">
						<?if($bFixed):?>
							<span class="search-icon-before">
								<?=TSolution::showIconSvg('', SITE_TEMPLATE_PATH.'/images/svg/Search_black.svg')?>
							</span>
						<?endif;?>
						...
						<div class="search-button-div">
							<?if($bFixed):?>
								...
								<span class="jqmClose top-close stroke-theme-hover inline-search-hide" title="<?=\Bitrix\Main\Localization\Loc::getMessage('CLOSE_BLOCK');?>"><?=TSolution::showIconSvg('', SITE_TEMPLATE_PATH.'/images/svg/Close.svg')?></span>
							<?else:?>
								<button class="btn btn-search btn--no-rippple" type="submit" name="s" title="<?=GetMessage("CT_BST_SEARCH_BUTTON")?>" value="<?=GetMessage("CT_BST_SEARCH_BUTTON")?>">
									<?=TSolution::showIconSvg('', SITE_TEMPLATE_PATH.'/images/svg/'.$svg.'.svg')?>
								</button>
							<?endif;?>
						</div>
					</form>

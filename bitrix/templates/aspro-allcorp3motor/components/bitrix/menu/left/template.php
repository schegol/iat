						...
						<li class="<?=($arSubItem["SELECTED"] ? "active opened" : "")?><?=($arSubItem["CHILD"] ? " child" : "")?>">
							<span class="bg-opacity-theme-parent-hover link-wrapper fill-theme-parent-all">
								<a href="<?=$arSubItem["LINK"]?>" class="dark_link font_<?=($arSubItem['PARAMS']['DEPTH_LEVEL'] == 2 ? 14 : 13);?> sublink <?=($arSubItem["CHILD"] ? " sublink--child" : "")?> <?=($arSubItem["SELECTED"] ? " link--active" : "")?>">
									<?if($arSubItem["CHILD"]):?>
										<?=TSolution::showIconSvg("down menu-arrow bg-opacity-theme-target fill-theme-target fill-dark-light-block", SITE_TEMPLATE_PATH.'/images/svg/Triangle_down.svg', '', '', true, false);?>
									<?endif;?>
									<?=$arSubItem["TEXT"]?>
								</a>
								<?if($arSubItem["CHILD"]):?>

			...
			<li class="mobilemenu__menu-item mobilemenu__menu-item--with-icon mobilemenu__menu-item--parent<?=(TSolution::isPersonalPage() ? ' mobilemenu__menu-item--selected' : '')?>">
				<div class="link-wrapper bg-opacity-theme-parent-hover fill-theme-parent-all color-theme-parent-all">
					...
					<a class="dark_link" href="<?=$link;?>" title="<?=Loc::getMessage('MY_CABINET')?>">
						<?=TSolution::showIconSvg('cabinet mobilemenu__menu-item-svg fill-theme-target', SITE_TEMPLATE_PATH."/images/svg/".($USER->IsAuthorized() ? 'User' : 'Lock')."_black.svg");?>
						<span class="font_15"><?=Loc::getMessage('MY_CABINET')?></span>
						<?if($bParent):?>
							<?=TSolution::showIconSvg(' down menu-arrow bg-opacity-theme-target fill-theme-target fill-dark-light-block', SITE_TEMPLATE_PATH.'/images/svg/Triangle_right.svg', '', '', true, false);?>
						<?endif;?>
					</a>
							...
							<li class="mobilemenu__menu-item<?=($arItem['SELECTED'] ? ' mobilemenu__menu-item--selected' : '')?><?=($bParent ? ' mobilemenu__menu-item--parent' : '')?>">
								<div class="link-wrapper bg-opacity-theme-parent-hover fill-theme-parent-all">
									...
									<a class="dark_link" href="<?=$arItem["LINK"]?>" title="<?=htmlspecialcharsbx($arItem["TEXT"])?>"<?=$arItem['ATTRIBUTE']?>>
										...
										<?if($bParent):?>
											<?=TSolution::showIconSvg('down menu-arrow bg-opacity-theme-target fill-theme-target', SITE_TEMPLATE_PATH.'/images/svg/Triangle_right.svg', '', '', true, false);?>
										<?endif;?>
									</a>
									<?if($bParent):?>
										<span class="toggle_block"></span>

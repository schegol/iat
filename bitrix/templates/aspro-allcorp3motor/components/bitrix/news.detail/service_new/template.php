...
<?elseif($arResult['FIELDS']['DETAIL_PICTURE']):?>
	...
	$bImageBefore = (isset($arResult['DISPLAY_PROPERTIES']['PHOTO_BEFORE']) && $arResult['DISPLAY_PROPERTIES']['PHOTO_BEFORE']['VALUE'] ? true : false);
	$templateData['IMG_TOP_SIDE'] = isset($arResult['PROPERTIES']['PHOTOPOS']) && $arResult['PROPERTIES']['PHOTOPOS']['VALUE_XML_ID'] == 'TOP_SIDE';
	$templateData['PHOTO_BEFORE'] = $arResult['DISPLAY_PROPERTIES']['PHOTO_BEFORE']['VALUE'];
	?>
	...
	<?if (!$templateData['IMG_TOP_SIDE']):?>
		...
		<?endif;?>
		<?
		$arParamsBlock['FILE'] = '/images/detail_single.php';
		$arParamsBlock['PARAMS'] = [
		...
		if($bImageBefore && !$templateData['BANNER_TOP_ON_HEAD']){
			$arParamsBlock['FILE'] = '/images/detail_before_after.php';
			$arParamsBlock['PARAMS']['BEFORE_IMG'] =  $templateData['PHOTO_BEFORE'];
		}
		?>
		<?\TSolution\Functions::showBlockHtml($arParamsBlock);?>
						...
						<?if ($bOrderButton):?>
							...
							<?if ($bAskButton):?>
								<div class="line-block__item">
									...
									data-param-id="<?=\TSolution::getFormID("question");?>" 
									data-name="question" 
									data-event="jqm" 
...
<?if($templateData['DETAIL_TEXT'] || $templateData['PREVIEW_TEXT']):?>
	<?$this->SetViewTarget('PRODUCT_DETAIL_TEXT_INFO');?>
		<div class="content new-service-content" itemprop="description">

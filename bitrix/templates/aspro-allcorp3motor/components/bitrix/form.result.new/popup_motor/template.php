		...
		<?if(!$arResult["FORM_NOTE"]){?>
			...
			<div class="form-body">
				<?if(is_array($arResult["QUESTIONS"])):?>
						<?$arDopParametersFields = [
							"COL_CLASS_COUN_SUMM" => 0,
							"COUNT_FIELDS_QUESTION" => count($arResult["QUESTIONS"]),
							"QUESTION_FIELDS_COUNTER" => 1
						];?>
					<?foreach($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion):?>
						<?
						if (in_array($arQuestion['STRUCTURE'][0]['FIELD_TYPE'], ['date', 'datetime'])) {
							$templateData['DATETIME'] = true;
						}
						?>
						<?if($arQuestion['HIDDEN']):?><div class='hidden'><?endif;?>
							<?TSolution::drawFormField($FIELD_SID, $arQuestion,"POPUP", [], $arDopParametersFields);?>
						<?if($arQuestion['HIDDEN']):?></div><?endif;?>
					<?endforeach;?>
					<?unset($arDopParametersFields);?>	
				<?endif;?>
			...
			<div class="form-footer">
				...
				<?endif;?>
				<div class="">
					<input type="submit" class="btn btn-default btn-lg 1" value="<?=$arResult["arForm"]["BUTTON"]?>" name="web_form_submit">
				</div>
			</div>

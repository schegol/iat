...
Loader::includeModule(TSolution::moduleID);

$arSite = CSite::GetByID($SITE_ID)->Fetch();

$arFrontParametrs = TSolution::GetFrontParametrsValues($SITE_ID, $SITE_DIR);
...
$currentBannerIndex = TSolution::getCurrentPresetBannerIndex($SITE_ID);
...
var arAsproOptions = window[solutionName] = ({
	...
	THEME: <?=CUtil::PhpToJSObject(
		array_merge(
			...
			array(
				...
				'IS_BASKET_PAGE' => TSolution::IsBasketPage($arFrontParametrs['BASKET_PAGE_URL']),
				'IS_ORDER_PAGE' => TSolution::IsBasketPage($arFrontParametrs['ORDER_PAGE_URL']),
	...
	PRESETS: <?=CUtil::PhpToJSObject(
		array(
			'VALUE' => TSolution::getCurrentPreset($SITE_ID),
			'LIST' => TSolution::$arPresetsList,
		)
	)?>,
	THEMATICS: <?=CUtil::PhpToJSObject(
		array(
			'VALUE' => TSolution::getCurrentThematic($SITE_ID),
			'LIST' => TSolution::$arThematicsList,
		)
	)?>,
	REGIONALITY: ({
	...
	COUNTERS: ({
		YANDEX_COUNTER: 1,
		GOOGLE_COUNTER: 1,
		YANDEX_ECOMERCE: '<?=Option::get(TSolution::moduleID, 'YANDEX_ECOMERCE', false, $SITE_ID)?>',
		GOOGLE_ECOMERCE: '<?=Option::get(TSolution::moduleID, 'GOOGLE_ECOMERCE', false, $SITE_ID)?>',
		TYPE: {
			ONE_CLICK: '<?=GetMessage('ONE_CLICK_BUY');?>',
			QUICK_ORDER: '<?=GetMessage('QUICK_ORDER');?>',
		},
		GOOGLE_EVENTS: {
			ADD2BASKET: '<?=trim(Option::get(TSolution::moduleID, 'BASKET_ADD_EVENT', 'addToCart', $SITE_ID))?>',
			REMOVE_BASKET: '<?=trim(Option::get(TSolution::moduleID, 'BASKET_REMOVE_EVENT', 'removeFromCart', $SITE_ID))?>',
			CHECKOUT_ORDER: '<?=trim(Option::get(TSolution::moduleID, 'CHECKOUT_ORDER_EVENT', 'checkout', $SITE_ID))?>',
			PURCHASE: '<?=trim(Option::get(TSolution::moduleID, 'PURCHASE_ORDER_EVENT', 'gtm.dom', $SITE_ID))?>',
		}
	}),

...
else
{
	...
	else
	{
				...
				<div class="blog-comment-fields">
						...
						<div class="blog-comment-field blog-comment-field-user">
							<div class="row form">
								<div class="col-md-6 col-sm-6">
									<div class="form-group fill-animate <?=($_SESSION["blog_user_name"] ? 'input-filed' : '');?>">
										<label class="font_13 color_666" for="user_name"><?=GetMessage("B_B_MS_NAME")?> <span class="required-star">*</span></label>
										<div class="input">
								...
								<div class="col-md-6 col-sm-6">
									<div class="form-group fill-animate <?=($_SESSION["blog_user_email"] ? 'input-filed' : '');?>">
						...
						<div class="row form">
							<div class="col-md-12">
								<div class="form-group fill-animate">
									<label class="font_13 color_666" for="user_sbj"><?=GetMessage("BPC_SUBJECT")?></label>
									<div class="input">
					...
					if($arResult["use_captcha"]===true)
					{
						...
						<div class="captcha-row clearfix fill-animate">
							<label class="font_13 color_666"><span><?=GetMessage("B_B_MS_CAPTCHA_SYM")?> <span class="required-star">*</span></span></label>
		...
		$prevTab = 0;
		function ShowComment($comment, $tabCount=0, $tabSize=1.55, $canModerate=false, $User=Array(), $use_captcha=false, $bCanUserComment=false, $errorComment=false, $arParams = array())
		{
			$comment["urlToAuthor"] = "";
			$comment["urlToBlog"] = "";
			...
			if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				if($tabCount <= 5)
					$paddingSize = 1.55 * $tabCount;
				elseif($tabCount > 5 && $tabCount <= 10)
					$paddingSize = 1.55 * 5 + ($tabCount - 5) * 1.5;
				elseif($tabCount > 10)
					$paddingSize = 1.55 * 5 + 1.5 * 5 + ($tabCount-10) * 1;

				if(($tabCount+1) <= 5)
					$paddingSizeNew = 1.55 * ($tabCount+1);
				elseif(($tabCount+1) > 5 && ($tabCount+1) <= 10)
					$paddingSizeNew = 1.55 * 5 + (($tabCount+1) - 5) * 1.5;
				elseif(($tabCount+1) > 10)
					$paddingSizeNew = 1.55 * 5 + 1.5 * 5 + (($tabCount+1)-10) * 1;
				$paddingSizeNew -= $paddingSize;

				if($prevTab > $tabCount)
					$prevTab = $tabCount;
				if($prevTab <= 5)
					$prevPaddingSize = 1.55 * $prevTab;
				elseif($prevTab > 5 && $prevTab <= 10)
					$prevPaddingSize = 1.55 * 5 + ($prevTab - 5) * 1.5;
				elseif($prevTab > 10)
					$prevPaddingSize = 1.55 * 5 + 1.5 * 5 + ($prevTab-10) * 1;

					$prevTab = $tabCount;
				?>
				<a name="<?=$comment["ID"]?>" id="c<?=$comment["ID"]?>"></a>
				<div class="blog-comment" style="padding-left:<?=$paddingSize?>em;">
				<?if($tabCount):?>
					<div class="blog-comment-arrow"><?=TSolution::showIconSvg('', SITE_TEMPLATE_PATH.'/images/svg/social/sub_comment_arrow.svg')?></div>
				<?endif;?>
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="blog-comment-content">
						...
						{
							?>
							<span class="blog-comment-answer"><a class="color-theme-hover" href="javascript:void(0)" onclick="return showComment('<?=$comment["ID"]?>')"><?=GetMessage("B_B_MS_REPLY")?></a></span>
						...
						{
							?>
							<span class="blog-comment-parent"><a class="color-theme-hover" href="#<?=$comment["PARENT_ID"]?>"><?=GetMessage("B_B_MS_PARENT")?></a></span>
							<span class="blog-vert-separator"></span>
							<?
						}
						?>
						<span class="blog-comment-link"><a class="color-theme-hover" href="#c<?=$comment["ID"]?>"><?=GetMessage("B_B_MS_LINK")?></a></span>
						<?
						if($comment["CAN_EDIT"] == "Y")
						{
							...
							<span class="blog-vert-separator"></span>
							<span class="blog-comment-edit"><a class="color-theme-hover" href="javascript:void(0)" onclick="return editComment('<?=$comment["ID"]?>')"><?=GetMessage("BPC_MES_EDIT")?></a></span>
							<?
						}
						if(strlen($comment["urlToShow"])>0)
						{
							...
							<span class="blog-comment-show">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a class="color-theme-hover" href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToShow"]."&".bitrix_sessid_get()?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_SHOW")?>">
								<?else:?>
									<a class="color-theme-hover" href="<?=$comment["urlToShow"]."&".bitrix_sessid_get()?>" title="<?=GetMessage("BPC_MES_SHOW")?>">
								<?endif;?>
						...
						if(strlen($comment["urlToHide"])>0)
						{
							...
							<span class="blog-comment-show">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a class="color-theme-hover" href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToHide"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=$_REQUEST["IBLOCK_ID"]?>&ELEMENT_ID=<?=$_REQUEST["ELEMENT_ID"]?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_HIDE")?>">
								<?else:?>
									<a class="color-theme-hover" href="<?=$comment["urlToHide"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=$_REQUEST["IBLOCK_ID"]?>&ELEMENT_ID=<?=$_REQUEST["ELEMENT_ID"]?>" title="<?=GetMessage("BPC_MES_HIDE")?>">
								<?endif;?>
						...
						if(strlen($comment["urlToDelete"])>0)
						{
							...
							<span class="blog-comment-delete">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a class="color-theme-hover" href="javascript:void(0)" onclick="if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) deleteComment('<?=$comment["urlToDelete"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=$_REQUEST["IBLOCK_ID"]?>&ELEMENT_ID=<?=$_REQUEST["ELEMENT_ID"]?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_DELETE")?>">
								<?else:?>
									<a class="color-theme-hover" href="javascript:if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$comment["urlToDelete"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=$_REQUEST["IBLOCK_ID"]?>&ELEMENT_ID=<?=$_REQUEST["ELEMENT_ID"]?>'" title="<?=GetMessage("BPC_MES_DELETE")?>">
								<?endif;?>
						...
						if(strlen($comment["urlToSpam"])>0)
						{
							?>
							<span class="blog-vert-separator"></span>
							<span class="blog-comment-delete blog-comment-spam"><a class="color-theme-hover" href="<?=$comment["urlToSpam"]?>" title="<?=GetMessage("BPC_MES_SPAM_TITLE")?>"><?=GetMessage("BPC_MES_SPAM")?></a></span>
							<?
						}
						if ($arParams["SHOW_RATING"] == "Y")
						{
							...
							|| (IntVal($_POST["edit_id"]) > 0 && IntVal($_POST["edit_id"]) == $comment["ID"] && $comment["CAN_EDIT"] == "Y")))
					{
						...
						ShowComment($commentPreview, (IntVal($_POST["edit_id"]) == $comment["ID"] && $comment["CAN_EDIT"] == "Y") ? $level : ($level+1), 1.55, false, Array(), false, false, false, $arParams);
					}
		...
		function RecursiveComments($sArray, $key, $level=0, $first=false, $canModerate=false, $User, $use_captcha, $bCanUserComment, $errorComment, $arSumComments, $arParams)
		{
			if(!empty($sArray[$key]))
			{
				foreach($sArray[$key] as $comment)
				{
					...
					ShowComment($comment, $level, 1.55, $canModerate, $User, $use_captcha, $bCanUserComment, $errorComment, $arParams);
					if(!empty($sArray[$comment["ID"]]))
					{
						foreach($sArray[$comment["ID"]] as $key1)
						{
							...
							ShowComment($key1, ($level+1), 1.55, $canModerate, $User, $use_captcha, $bCanUserComment, $errorComment, $arParams);
		...
		if($arResult["is_ajax_post"] != "Y")
		{
			if($arResult["CanUserComment"])
			{
				...
				?>
				<div class="blog-add-comment"><a class="btn btn-default btn-lg btn-wide btn-transparent-border" href="javascript:void(0)"><?=GetMessage("B_B_MS_ADD_COMMENT")?></a></div>
				<a name="0"></a>
				<?

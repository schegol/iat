	...
	for($index = 0; $index < $cnt; ++$index){
		...
		if($arItem["LINK"] <> "" && $arItem['LINK'] != GetPagePath() && $arItem['LINK']."index.php" != GetPagePath() || $arSubSections){
			...
			if($arSubSections){
				...
				$strReturn .=($arSubSections ? '<span itemprop="name" class="breadcrumbs__item-name font_13">'.$title.'</span><span class="breadcrumbs__arrow-down '.(!$bLast ? 'colored_theme_hover_bg-el-svg' : '').'">'.TSolution::showIconSvg("arrow", SITE_TEMPLATE_PATH."/images/svg/trianglearrow_down.svg").'</span>' : '<span>'.$title.'</span>');

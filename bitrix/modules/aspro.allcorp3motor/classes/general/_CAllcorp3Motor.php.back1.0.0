...
use \Bitrix\Main\Application,
	...
	\Aspro\Allcorp3Motor\Functions\CAsproAllcorp3Admin as Admin,
	CAllcorp3MotorRegionality as Regionality,
	Aspro\Allcorp3Motor\Functions\Extensions,
	\Aspro\Allcorp3Motor\Property\ListWebForms,
	CAllcorp3MotorConst as CConst;
...
class CAllcorp3Motor{
	const partnerName = CConst::PARTNER_NAME;
	const solutionName = CConst::SOLUTION_NAME;
	const themesSolutionName = CConst::THEMES_SOLUTION_NAME;
	const templateName = CConst::TEMPLATE_NAME;
	const moduleID = CConst::MODULE_ID;
	...
	public static function GetSections($arItems, $arParams)
	{
		...
		if (is_array($arItems) && $arItems) {
			...
			if ($arSectionsIDs) {
				$arCacheParams = array(
					'TAG' => Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']),
					'GROUP' => array('ID'),
					'MULTI' => 'N'
				);
				...
				$arSections['ALL_SECTIONS'] = Cache::CIBLockSection_GetList(
					array(
						'SORT' => 'ASC',
						'NAME' => 'ASC',
						'CACHE' => $arCacheParams,
				...
				foreach ($arSections['ALL_SECTIONS'] as $key => $arSection) {
					if ($arSection['DEPTH_LEVEL'] > 1) {
						...
						$arParentSection = Cache::CIBLockSection_GetList(
							array(
								'SORT' => 'ASC',
								'NAME' => 'ASC',
								'CACHE' => $arCacheParams,
				...
				if ($bCheckRoot) {
					...
					$arRootSections = Cache::CIBLockSection_GetList(
						array(
							'SORT' => 'ASC',
							'NAME' => 'ASC',
							'CACHE' => $arCacheParams,
	...
	{
		if ($handler = Functions::getCustomFunc(__FUNCTION__)) {
			call_user_func_array($handler, [$arParams, &$aMenuLinksExt, $bMenu]);
			return;
		}
		$catalog_id = \Bitrix\Main\Config\Option::get(self::moduleID, 'CATALOG_IBLOCK_ID', Cache::$arIBlocks[SITE_ID]['aspro_'.self::themesSolutionName.'_catalog']['aspro_'.self::themesSolutionName.'_catalog'][0]);
		$bIsCatalog = $arParams['IBLOCK_ID'] == $catalog_id;

		$arParams['CATALOG_IBLOCK_ID'] = $catalog_id;
		$arParams['IS_CATALOG_IBLOCK'] = $bIsCatalog;

		foreach(GetModuleEvents(self::moduleID, 'BeforeAsproGetMenuChildsExt', true) as $arEvent) // event for manipulation store quantity block
		...
		$arSectionFilter = array(
			...
			'GLOBAL_ACTIVE' => 'Y',
			'ACTIVE_DATE' => 'Y',
			'<DEPTH_LEVEL' => \Bitrix\Main\Config\Option::get(self::moduleID, "MAX_DEPTH_MENU", 2),
		);
		...
		{
			$arSections = Cache::CIBlockSection_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), $arSectionFilter, false, $arSectionSelect);
		...
		if(!$bIsCatalog) {
			if($arParams['MENU_PARAMS']['MENU_SHOW_ELEMENTS'] == 'Y'){
				...
				$arItems = Cache::CIBlockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), $arElementFilter, false, false, $arElementSelect);
				...
				if($arParams['MENU_PARAMS']['MENU_SHOW_SECTIONS'] == 'Y'){
					...
					$arItemsRoot = Cache::CIBlockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'MULTI' => 'Y')), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ACTIVE' => 'Y', 'ACTIVE_DATE' => 'Y', 'SECTION_ID' => 0));
					$arItems = array_merge((array)$arItems, (array)$arItemsRoot);
				}
			}
		}

		foreach(GetModuleEvents(self::moduleID, 'OnAsproGetMenuChildsExt', true) as $arEvent) // event for manipulation store quantity block
			ExecuteModuleEventEx($arEvent, array($arParams, &$aMenuLinksExt, $bMenu));
		...
		foreach(GetModuleEvents(self::moduleID, 'AfterAsproGetMenuChildsExt', true) as $arEvent) // event for manipulation store quantity block
			ExecuteModuleEventEx($arEvent, array($arParams, &$aMenuLinksExt, $bMenu));
	}
	...
	public static function replaceMenuChilds(&$arResult, $arParams)
	{
		...
		$menuIblockId = Cache::$arIBlocks[SITE_ID]['aspro_'.self::themesSolutionName.'_catalog']['aspro_'.self::themesSolutionName.'_megamenu'][0];
		if($menuIblockId){
			$arMenuSections = Cache::CIblockSection_GetList(
				array(
					'SORT' => 'ASC',
					'NAME' => 'ASC',
					'CACHE' => array(
						'TAG' => Cache::GetIBlockCacheTag($menuIblockId),
	...
	public static function ShowPageType($type = 'indexblocks', $subtype = '', $template = '', $bRestart = '', $bLoadAjax = false)
	{
		...
		if ((is_array($arTheme) && $arTheme) || $bShowBlock) {
			...
			if($type === 'footer'){
				...
				$checkProperty = Functions::checkProperty($APPLICATION->GetProperty('NAME_BUTTON_DOWNLOAD'), $APPLICATION->GetProperty('URL_BUTTON_DOWNLOAD'));
				if($checkProperty){
	...
	public static function ShowLogo($arParams = array()){
		...
		$arImg = unserialize(Option::get(self::moduleID, $logoOptionCode, serialize(array()))) ?: ($isWhite ? unserialize(Option::get(self::moduleID, 'LOGO_IMAGE', serialize(array()))) : array());
	...
	public static function GetBackParametrsValues($SITE_ID, $SITE_DIR = '', $bFromStatic = true){
		...
		if(!$arValues){
			...
			foreach(GetModuleEvents(self::moduleID, 'OnAsproGetBackParametrsValuesWidget', true) as $arEvent) // event for manipulation widget
				ExecuteModuleEventEx($arEvent, array($SITE_ID));
					...
					if($arWebForms[$SITE_ID] = ListWebForms::getWebForms(array($SITE_ID))){
						...
						foreach($arWebForms[$SITE_ID] as $type => $arWebFormsOfType){
							foreach($arWebFormsOfType as $code => $name){
								...
								foreach(GetModuleEvents(self::moduleID, 'OnAsproGetBackParametrsValuesFormOptions', true) as $arEvent) // event for manipulation web form options in params
									ExecuteModuleEventEx($arEvent, array(&$arFormsOptions, $code));
							}
						}
						...
						foreach($arBlock['OPTIONS'] as $optionCode => $arOption)
						{
							if($arOption['TYPE'] !== 'note' && $arOption['TYPE'] !== 'includefile'){
								if($arOption['TYPE'] === 'array'){
									...
									Admin::getBackParams($arValues, $arDefaultValues, $arOption, $SITE_ID);
	...
	public static function GetFrontParametrsValues($SITE_ID, $SITE_DIR = '', $bFromStatic = true){
		...
		$GLOBALS['_USE_LAZY_LOAD_'.mb_strtoupper(self::themesSolutionName).'_'] = $arValues['USE_LAZY_LOAD'] === 'Y';
							...
							<?foreach($arOption['LIST'] as $key => $value):?>
								...
								if($arOption['SUB_PARAMS'][$key]):?>
									<div class="parent-wrapper js-sub block_<?=$key.'_'.$optionsSiteID;?>" <?=($optionVal == $key ? "style='display:block;'" : "")?>>
										...
										$arNewOptions = Functions::getCustomBlocks($optionsSiteID);
			...
			<?if(Loader::IncludeModule('iblock')):?>
				<div colspan="2">
					...
					$arFilter = array(
						...
						"TYPE" => "aspro_".self::themesSolutionName."_form"
					);
					$rsItems = CIBlock::GetList($arSort, $arFilter);
					while($arItem = $rsItems->Fetch()){
						if($arItem["CODE"] != "aspro_".self::themesSolutionName."_example" && $arItem["CODE"] != "aspro_".self::themesSolutionName."_order_page")
						{
		...
		<?elseif($optionType == "note"):?>
			<?if($optionCode == 'GOALS_NOTE')
			{
				...
				if(\Bitrix\Main\Loader::includeModule('form'))
				{
					if($optionsSiteID)
					{
						if($arForms = Cache::CForm_GetList($by = array('by' => 's_id', 'CACHE' => array('TAG' => 'forms')), $order = 'asc', array('SITE' => $optionsSiteID, 'SITE_EXACT_MATCH' => 'Y'), $is_filtered))
						{
						...
						<select  <?=((isset($arOption['DEPENDENT_PARAMS']) && $arOption['DEPENDENT_PARAMS']) ? "class='depend-check'" : "");?> data-site="<?=$optionsSiteID?>" name="<?=htmlspecialcharsbx($optionCode)."_".$optionsSiteID?>" <?=$optionController?> <?=$optionDisabled?>>
							<?if($bIBlocks)
							{
								foreach($arIBlocks as $key => $arValue) {
									$selected="";
									if (!$optionVal || is_array($optionVal)) {
										if(
											isset($arOption['DEFAULT'])
											&& is_array($arOption['DEFAULT'])
											&& isset($arOption['DEFAULT']['TYPE'])
											&& isset($arOption['DEFAULT']['CODE'])
											&& in_array($optionsSiteID, (array)Cache::$arIBlocksInfo[$key]['LID'])
											&& $arOption['DEFAULT']['TYPE'] == Cache::$arIBlocksInfo[$key]['IBLOCK_TYPE_ID']
											&& $arOption['DEFAULT']['CODE'] == Cache::$arIBlocksInfo[$key]['CODE']
										) {
											$selected="selected";
										}
										else {
											if ($arValue["CODE"] == "aspro_".self::themesSolutionName."_catalog") {
												$selected = "selected";
											}
										}
	...
	public static function checkBgImage($siteID){
		...
		if($arBanner === NULL)
		{
			$bgIbockID = (Cache::$arIBlocks[$siteID]['aspro_'.self::themesSolutionName.'_content']['aspro_'.self::themesSolutionName.'_bg_images'][0] ? Cache::$arIBlocks[$siteID]['aspro_'.self::themesSolutionName.'_content']['aspro_'.self::themesSolutionName.'_bg_images'][0] : Cache::$arIBlocks[$siteID]['aspro_'.self::themesSolutionName.'_adv']['aspro_'.self::themesSolutionName.'_bg_images'][0]);
			...
			$arItems = Cache::CIBLockElement_GetList(array('SORT' => 'ASC', 'CACHE' => array('TAG' => $bgIbockID)), $arFilterBanner, false, false, array('ID', 'NAME', 'PREVIEW_PICTURE', 'PROPERTY_URL', 'PROPERTY_FIXED_BANNER', 'PROPERTY_URL_NOT_SHOW'));
			$arBanner = array();
	...
	/*public static function getCompareItems(){
		...
		if(!defined('ADMIN_SECTION') && !CSite::inDir(SITE_DIR.'/ajax/') && \self::GetFrontParametrValue('CATALOG_COMPARE') === 'Y'):?>
	...
	public static function SetMeta(){
		...
		if($arRegion)
		{
			...
			if($arTagSeoMarks)
				Regionality::addSeoMarks($arTagSeoMarks);
		}
	...
	public static function getSectionInheritedUF($arParams = array()){
		...
		if($arParams){
			...
			if($sectionId && $arSelect){
				...
				$arSection = Cache::CIBlockSection_GetList(
					array(
						'CACHE' => array(
							'MULTI' => 'N',
	...
	public static function ShowBasketLink($class_link = 'top-btn hover', $class_icon = '', $txt = '', $show_price = false, $bMobile = false){
		static $basket_call;
		...
		if(
			...
			<div class="mobilemenu__menu mobilemenu__menu--cart">
				<ul class="mobilemenu__menu-list">
					<li class="mobilemenu__menu-item mobilemenu__menu-item--with-icon<?=(self::IsBasketPage() ? ' mobilemenu__menu-item--selected' : '')?>">
	...
	public static function ShowMobileMenuCompare(){
		...
		if(
			...
			<div class="mobilemenu__menu mobilemenu__menu--compare">
				<ul class="mobilemenu__menu-list">
					<li class="mobilemenu__menu-item mobilemenu__menu-item--with-icon<?=(self::IsComparePage() ? ' mobilemenu__menu-item--selected' : '')?>">
			...
			<li class="mobilemenu__menu-item mobilemenu__menu-item--with-icon mobilemenu__menu-item--parent">
				<div class="link-wrapper bg-opacity-theme-parent-hover fill-theme-parent-all color-theme-parent-all">
					<a class="dark_link" href="<?=$href?>" rel="nofollow">
						...
						<?endif;?>
						<?=self::showIconSvg(' down menu-arrow bg-opacity-theme-target fill-theme-target fill-dark-light-block', SITE_TEMPLATE_PATH.'/images/svg/Triangle_right.svg', '', '', true, false);?>
					</a>
				...
				<ul class="mobilemenu__menu-dropdown dropdown">
					<li class="mobilemenu__menu-item mobilemenu__menu-item--back">
						<div class="link-wrapper stroke-theme-parent-all colored_theme_hover_bg-block animate-arrow-hover color-theme-parent-all">
							<a class="arrow-all arrow-all--wide stroke-theme-target" href="" rel="nofollow">
								<?=self::showIconSvg(' arrow-all__item-arrow', SITE_TEMPLATE_PATH.'/images/svg/Arrow_lg.svg');?>
					...
					<?if($arOptions['CALLBACK']):?>
						<li class="mobilemenu__menu-item mobilemenu__menu-item--callback">
							<div class="animate-load btn btn-default btn-transparent-border btn-wide" data-event="jqm" data-param-id="<?=self::getFormID("callback");?>" data-name="callback">
								<?=GetMessage('CALLBACK')?>
							</div>
						</li>
					<?endif;?>
	...
	public static function checkContentBlock($file, $prop = 'PROPERTY_ADDRESS_VALUE'){
		global $arRegion;
		if((self::checkContentFile($file) && !$arRegion) || ($arRegion && $arRegion[$prop]))
			return true;
		return false;
	}
	...
	public static function CheckAdditionalChainInMultiLevel(&$arResult, &$arParams, &$arElement){
		...
		if($arParams['INCLUDE_IBLOCK_INTO_CHAIN'] == 'Y' && isset(Cache::$arIBlocksInfo[$arParams['IBLOCK_ID']]['NAME']))
			$APPLICATION->AddChainItem(Cache::$arIBlocksInfo[$arParams['IBLOCK_ID']]['NAME'], $arElement['~LIST_PAGE_URL']);

		if($arParams['ADD_SECTIONS_CHAIN'] == 'Y')
		{
			if($arSection = Cache::CIBlockSection_GetList(array('CACHE' => array('TAG' => Cache::GetIBlockCacheTag($arElement['IBLOCK_ID']), 'MULTI' => 'N')), self::GetCurrentSectionFilter($arResult['VARIABLES'], $arParams), false, array('ID', 'NAME')))
			{
	...
	public static function CheckDetailPageUrlInMultilevel(&$arResult){
		if($arResult['ITEMS']){
			...
			$arItems = Cache::CIBLockElement_GetList(array('CACHE' => array('TAG' => Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']), 'GROUP' => array('ID'), 'MULTI' => 'N')), array('ID' => $arItemsIDs), false, false, array('ID', 'IBLOCK_SECTION_ID', 'DETAIL_PAGE_URL'));
	...
	public static function Start($siteID = 's1'){
		...
		if(CModule::IncludeModuleEx(self::moduleID) == 1)
		{
			...
			$APPLICATION->AddHeadString('
				<script data-skip-moving="true" src="/bitrix/js/'.self::partnerName.'.'.self::themesSolutionName.'/jquery/jquery-2.1.3.min.js"></script>
			...
			if($arTheme['USE_REGIONALITY'] == 'Y'){
				$arRegion = Regionality::getCurrentRegion(); //get current region from regionality module
			}
	...
	public static function ShowPageProps($prop){
		...
		global $APPLICATION;
		$APPLICATION->AddBufferContent(array(__CLASS__, "GetPageProps"), $prop);
	}
	...
	public static function ShowListRegions($arParams = array()){?>
		...
		<?$frame->begin();?>
		<?$APPLICATION->IncludeComponent(
			"aspro:regionality.list." . self::themesSolutionName,
   ...
   protected static function IsComponentTemplateHasModuleElementsPageBlocksParam($templateName, $arExtParams = array()){
    ...
    public static function GetItemsYear($arParams){
    	$arResult = array();
    	$arItems = Cache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => Cache::GetIBlockCacheTag($arParams['IBLOCK_ID']))), array('IBLOCK_ID' => $arParams['IBLOCK_ID'], 'ACTIVE' => 'Y'), false, false, array('ID', 'NAME', 'ACTIVE_FROM'));
		if($arItems)
		{
			foreach($arItems as $arItem)
			{
	...
	public static function getConditionClass(){
		...
		$class = $APPLICATION->AddBufferContent(array(__CLASS__, 'showPageClass'));
		...
		foreach(GetModuleEvents(self::moduleID, 'OnAsproGetConditionClass', true) as $arEvent) // event for manipulation body classes
			ExecuteModuleEventEx($arEvent, array($class, $arSiteThemeOptions));
				...
				<div id="mobilephones" class="phones__dropdown">
					<div class="mobilephones__menu-dropdown dropdown dropdown--relative">
						...
						<span class="mobilephones__close stroke-theme-hover" title="<?=\Bitrix\Main\Localization\Loc::getMessage('CLOSE_BLOCK');?>">
							<?=self::showIconSvg('', SITE_TEMPLATE_PATH.'/images/svg/Close.svg')?>
						</span>
						...
						<div class="dropdown dropdown--relative">
							...
							<?if($bDropdownCallback != 'N'):?>
								<div class="phones__dropdown-item callback-item">
									<div class="animate-load btn btn-default btn-wide" data-event="jqm" data-param-id="<?=self::getFormID("callback");?>" data-name="callback">
										<?=GetMessage('CALLBACK')?>
									</div>
								</div>
							<?endif;?>
	...
	public static function showRightDok(){
		...
		foreach(GetModuleEvents(self::moduleID, 'OnAsproShowRightDokWidget', true) as $arEvent) // event for manipulation widget
			$widgetHtml = ExecuteModuleEventEx($arEvent, array());

		if($callbackBlock || $questionBlock || $reviewBlock || $compareBlock || $widgetHtml):?>
			<div class="right_dok">
				<?if($compareBlock):?>
				...
				<?if($callbackBlock):?>
					<span class="link fill-theme-hover" title="<?=GetMessage("S_CALLBACK")?>">
						<span class="animate-load" data-event="jqm" data-param-id="<?=self::getFormID("callback");?>" data-name="callback"><?=self::showIconSvg("call", SITE_TEMPLATE_PATH."/images/svg/back_call.svg");?></span>
					</span>
				<?endif;?>
				<?if($reviewBlock):?>
					<span class="link fill-theme-hover" title="<?=GetMessage("S_FEEDBACK")?>">
						<span class="animate-load" data-event="jqm" data-param-id="<?=self::getFormID("feedback");?>" data-name="review"><?=self::showIconSvg("review", SITE_TEMPLATE_PATH."/images/svg/send_review.svg");?></span>
					</span>
				<?endif;?>
				<?if($questionBlock):?>
					<span class="link fill-theme-hover" title="<?=GetMessage("S_QUESTION")?>">
						<span class="animate-load" data-event="jqm" data-param-id="<?=self::getFormID("question");?>" data-name="question"><?=self::showIconSvg("ask", SITE_TEMPLATE_PATH."/images/svg/ask_question.svg");?></span>
					</span>
				<?endif;?>
				<?echo $widgetHtml;?>
			</div>
	...
	public static function getSectionsIds_NotInRegion($iblockId = false, $regionId = false){
		...
		if(!$iblockId){
			$iblockId = Cache::$arIBlocks[SITE_ID]['aspro_'.self::themesSolutionName.'_catalog']['aspro_'.self::themesSolutionName.'_catalog'][0];
		}
				...
				if($regionId){
					...
					if(!isset($arCache[$iblockId][$regionId])){
						if($arSections = Cache::CIBLockSection_GetList(
							...
							if($arSectionsIds){
								if($arSectionsIds_ = Cache::CIBLockSection_GetList(
									array(
										'CACHE' => array(
											'TAG' => Cache::GetIBlockCacheTag($iblockId),
											'MULTI' => 'Y',
	...
	public static function checkElementsIdsInRegion(&$arIds, $iblockId = false, $regionId = false){
		...
		if($bFilterItem && $arIds){
			...
			if($iblockId){
				...
				if($regionId){
					if($arSectionsIds = self::getSectionsIds_NotInRegion($arFilter['IBLOCK_ID'], $regionId)){
						$arIds = Cache::CIBLockElement_GetList(
							array(
								'CACHE' => array(
									'TAG' => Cache::GetIBlockCacheTag($iblockId),
									'RESULT' => array('ID'),
	...
	public static function drawFormField($FIELD_SID, $arQuestion, $type = 'POPUP', $arParams = array(), &$arDopParametersFields = []) {
		foreach (GetModuleEvents(self::moduleID, 'BeforeAsproDrawFormField', true) as $arEvent) // event for manipulation params of drawed form field
			ExecuteModuleEventEx($arEvent, array($FIELD_SID, &$arQuestion, &$type, &$arParams));
		...
		preg_match('/\bcol-(\d+)\b/', $arQuestion['STRUCTURE'][0]['FIELD_PARAM'], $colClassCount);
		$colClassCount = ($colClassCount[1] && $colClassCount[1] <= 12) ? $colClassCount[1] : 12;?>

		<?if($arQuestion['STRUCTURE'][0]['FIELD_TYPE'] == 'hidden'):?>
			<?
			if ($FIELD_SID == 'TOTAL_SUMM' && $arParams['TOTAL_SUMM']) {
					$arQuestion["HTML_CODE"] = str_replace('value="', 'value="'.$arParams['TOTAL_SUMM'], $arQuestion["HTML_CODE"]);
				}
			?>
			<?=$arQuestion["HTML_CODE"];?>
		<?else:?>
				<?$checkSum = (($arDopParametersFields["COL_CLASS_COUN_SUMM"] + $colClassCount) > 12);?>
				<?if($checkSum):?>
					</div>
				<?endif;?>
				<?if(!$arDopParametersFields["COL_CLASS_COUN_SUMM"] || $checkSum):?>
					<div class="row" data-SID="<?=$FIELD_SID?>">
				<?endif;?>
				<div class="col-md-<?=$colClassCount?> <?=(in_array($arQuestion['STRUCTURE'][0]['FIELD_TYPE'], array("checkbox", "radio")) ? "style_check bx_filter" : "");?>">
			...
			<?
			$arDopParametersFields["COL_CLASS_COUN_SUMM"] += $colClassCount;
			$lastQuestionField = $arDopParametersFields["COUNT_FIELDS_QUESTION"] == ($arDopParametersFields["QUESTION_FIELDS_COUNTER"]);
			?>
			<?if ($arDopParametersFields["COL_CLASS_COUN_SUMM"] >= 12 || $lastQuestionField):
				$arDopParametersFields["COL_CLASS_COUN_SUMM"] = 0;?>
				</div>
			<?endif;?>
			<?$arDopParametersFields["QUESTION_FIELDS_COUNTER"]++;?>

		<?endif;?>
	<?}


	public static function getFormID($shortCode = '', $site = SITE_ID){
		foreach(GetModuleEvents(self::moduleID, 'BeforeAsproGetFormID', true) as $arEvent) // event for manipulation form id
			ExecuteModuleEventEx($arEvent, array(&$shortCode));

		$form_id = 0;
		if($shortCode)
		{
			if(strpos($shortCode, self::partnerName . '_' . self::themesSolutionName) !== false){
				$code = $shortCode;
			}else{
				$code = self::partnerName . '_' . self::themesSolutionName . '_' . $shortCode;
			}			
			if(self::GetFrontParametrValue('USE_BITRIX_FORM') == 'Y' && \Bitrix\Main\Loader::includeModule('form'))
			{
				...
				if($item = $rsForm->Fetch())
					...
					$form_id = Cache::$arIBlocks[$site]["aspro_".self::themesSolutionName."_form"][$code][0];
			}
			else
			{
				$form_id = Cache::$arIBlocks[$site]["aspro_".self::themesSolutionName."_form"][$code][0];
	...
	public static function get_banners_position($position){
		...
		if ($arTheme["ADV_".$position] == 'Y') {
			...
			$APPLICATION->IncludeComponent(
				...
				"banners",
				array(
					"IBLOCK_TYPE" => "aspro_".self::themesSolutionName."_adv",
					"IBLOCK_ID" => Cache::$arIBlocks[SITE_ID]["aspro_".self::themesSolutionName."_adv"]["aspro_".self::themesSolutionName."_banners"][0],
					"POSITION"	=> $position,
	...
	public static function setGenerateColorsOn($siteId = 's1') {
		\Bitrix\Main\Config\Option::set(self::moduleID, 'NeedGenerateThemes', 'Y', $siteId);
		\Bitrix\Main\Config\Option::set(self::moduleID, 'NeedGenerateCustomTheme', 'Y', $siteId);
	}
	...
	public static function setCatalogSectionDescription(array $arParams){
		$siteId = isset($siteId) && strlen($siteId) ? $siteId : (defined('SITE_ID') ? SITE_ID : '');

...
class CAllcorp3MotorRegionality
{
	...
	public static function checkUseRegionality()
	{
		...
		{
			return Solution::GetFrontParametrValue('USE_REGIONALITY');
		}
		return 'N';
	}
	...
	public static function getRegionIBlockID()
	{
		...
		{
			if(isset(Cache::$arIBlocks[SITE_ID]['aspro_'.Solution::themesSolutionName.'_regionality']['aspro_'.Solution::themesSolutionName.'_regions'][0]) && Cache::$arIBlocks[SITE_ID]['aspro_'.Solution::themesSolutionName.'_regionality']['aspro_'.Solution::themesSolutionName.'_regions'][0])
			{
				$iRegionIBlockID = Cache::$arIBlocks[SITE_ID]['aspro_'.Solution::themesSolutionName.'_regionality']['aspro_'.Solution::themesSolutionName.'_regions'][0];
			}
			else
			{
				return;
	...
	public static function replaceSeoMarks()
	{
		...
		if($arRegion && $page_title)
		{
			...
			{
				$bShowSiteName = (\Bitrix\Main\Config\Option::get(Solution::moduleID, "HIDE_SITE_NAME_TITLE", "N") == "N");
	...
	public static function getRegions()
	{
		...
		if($arRegions === NULL)
		{
			...
			if($iRegionIBlockID = self::getRegionIBlockID())
			{
				...
				$cache_id = 'aspro_'.Solution::themesSolutionName.'_regions'.$iRegionIBlockID;
				...
				else
				{
					...
					foreach(GetModuleEvents(Solution::moduleID, 'OnAsproRegionalityAddSelectFieldsAndProps', true) as $arEvent) // event for add to select in region getlist elements
						ExecuteModuleEventEx($arEvent, array(&$arSelect));

					// $arItems = Cache::CIBLockElement_GetList(array('SORT' => 'ASC', 'NAME' => 'ASC', 'CACHE' => array('TAG' => Cache::GetIBlockCacheTag($iRegionIBlockID), 'GROUP' => 'ID')), $arFilter, false, false, $arSelect);
					...
					foreach(GetModuleEvents(Solution::moduleID, 'OnAsproRegionalityGetElements', true) as $arEvent) // event for manipulation with region elements

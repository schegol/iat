...
use CAllcorp3MotorCache as Cache;

class CAllcorp3MotorCache {
	static public $arIBlocks = NULL;
	static public $arIBlocksInfo = NULL;
	static public $bEnabled = true;
	static public $className = 'CAllcorp3MotorCache';
			...
			if($arGroupBy === array()){
				...
				if($bGetDetailPageUrlsArray){
					...
					foreach($arByIBlock as $IBLOCK_ID => $arIB){
						...
						if($arSectionIDs){
							$arSections = self::CIBlockSection_GetList(array("CACHE" => array("TAG" => self::GetIBlockCacheTag($IBLOCK_ID), "MULTI" => "N", "GROUP" => array("ID"))), array("ID" => $arSectionIDs), false, array("ID", "CODE", "EXTERNAL_ID", "IBLOCK_ID"));
						}
						foreach($arIB as $SECTION_ID => $arIDs){
							if($arIDs){
								$rsElements = CIBlockElement::GetList(array(), array("ID" => $arIDs), false, false, array("ID", "DETAIL_PAGE_URL", "CANONICAL_PAGE_URL"));
								$rsElements->SetUrlTemplates(self::$arIBlocksInfo[$IBLOCK_ID]["DETAIL_PAGE_URL"]);
	...
	public static function CSaleBasket_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
		...
		if(!strlen($cachePath)){
			$cachePath = "/".self::$className."/sale/CSaleBasket_GetList/".$cacheTag."/";
		}
		$obCache = new CPHPCache();
	...
	public static function CCatalogStore_GetList($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
		...
		if(!strlen($cachePath)){
			$cachePath = "/".self::$className."/catalog/CCatalogStore_GetList/".$cacheTag."/";
		}
		$obCache = new CPHPCache();
	...
	public static function CForumMessage_GetListEx($arOrder = array("SORT" => "ASC"), $arFilter = array(), $arGroupBy = false, $iNum = 0, $arSelectFields = array(), $cacheTag = "", $cacheTime = 36000000, $cachePath = ""){
		...
		if(!strlen($cachePath)){
			$cachePath = "/".self::$className."/forum/CForumMessage_GetListEx/".$cacheTag."/";
		}
		$obCache = new CPHPCache();
	...
	private static function _MakeResultTreeArray($arParams, &$arItem, &$arItemResval, &$to){
		if($arParams["GROUP"]){
			...
			if (isset($arParams["MULTI"]) && $arParams["MULTI"] === "Y") {
				$to[] = $arItemResval;
			}
			elseif (isset($arParams["MULTI"]) && $arParams["MULTI"] === "YM") {
				if($to){
	...
	private static function _InitCacheParams($moduleName, $functionName, $arCache){
		...
		if(!strlen($cachePath)){
			$cachePath = "/".self::$className."/".$moduleName."/".$functionName."/".$cacheTag."/";
		}
	...
	public static function GetIBlockCacheTag($IBLOCK_ID){
		if(!$IBLOCK_ID){
			...
			return @self::$arIBlocksInfo[$IBLOCK_ID]["CODE"].$IBLOCK_ID;
		}
	}
	...
	public static function ClearTagIBlockElement($arFields){
		...
		if($arFields["IBLOCK_ID"]){
			$CACHE_MANAGER->ClearByTag(self::GetIBlockCacheTag($arFields["IBLOCK_ID"]));
	...
	public static function ClearTagIBlockSectionBeforeDelete($ID){
		global $CACHE_MANAGER;
		if ($ID > 0) {
			if ($IBLOCK_ID = self::CIBlockSection_GetList(array("CACHE" => array("MULTI" => "N", 'RESULT' => array('IBLOCK_ID'))), array("ID" => $ID), false, array("IBLOCK_ID"), true)) {
				$CACHE_MANAGER->ClearByTag(
					self::GetIBlockCacheTag($IBLOCK_ID)
				);
			}
		}
	}
...
// initialize self::$arIBlocks array

...
use \Bitrix\Main\Localization\Loc,
	...
	Aspro\Allcorp3Motor\Functions\CAsproAllcorp3ReCaptcha as ReCaptcha,
	Aspro\Allcorp3Motor\Functions\CAsproAllcorp3 as Functions,
	Aspro\Allcorp3Motor\Functions\ThematicParameters as ThematicParameters,
	Aspro\Allcorp3Motor\Notice;

Loc::loadMessages(__FILE__);

class CAllcorp3MotorEvents{
	...
	public static function OnRegionUpdateHandler($arFields){
		...
		{
			if(isset(Cache::$arIBlocks[$arIBlock['LID']]['aspro_'.Solution::themesSolutionName.'_regionality']['aspro_'.Solution::themesSolutionName.'_regions'][0]) && Cache::$arIBlocks[$arIBlock['LID']]['aspro_'.Solution::themesSolutionName.'_regionality']['aspro_'.Solution::themesSolutionName.'_regions'][0])
				$iRegionIBlockID = Cache::$arIBlocks[$arIBlock['LID']]['aspro_'.Solution::themesSolutionName.'_regionality']['aspro_'.Solution::themesSolutionName.'_regions'][0];
			else
				return;
	...
	public static function OnBeforeUserUpdateHandler(&$arFields){
		...
		$siteID = SITE_ID;
		$sChangeLogin = COption::GetOptionString(Solution::moduleID, 'LOGIN_EQUAL_EMAIL', 'N', $siteID);
		...
		if(strlen($arFields['NAME']) && !strlen($arFields['LAST_NAME']) && !strlen($arFields['SECOND_NAME']))
		{
			if($siteID == 'ru')
				$siteID = 's1';
			if($bAdminSection)
				$bOneFIO = COption::GetOptionString(Solution::moduleID, 'PERSONAL_ONEFIO', 'Y', $siteID);
			else
			{
	...
	public static function OnBeforeEventAddHandler(&$event, &$lid, &$arFields, &$message_id){
		if(\Bitrix\Main\Loader::includeModule(self::MODULE_ID))
		{
			if($arCurrentRegion = Regionality::getCurrentRegion())
			{
				...
				$arFields['REGION_EMAIL'] = implode(', ', (array)$arCurrentRegion['PROPERTY_EMAIL_VALUE']);
				$arFields['REGION_PHONE'] = implode(', ', (array)$arCurrentRegion['PHONES']);
				...
				foreach(Regionality::$arSeoMarks as $mark => $field)
				{
	...
	public static function OnEndBufferContentHandler(&$content)
	{
		...
		{			
			...
			// lazyload
			global $arTheme;
			if($GLOBALS['_USE_LAZY_LOAD_'.mb_strtoupper(Solution::themesSolutionName).'_']){
			...
			//process recaptcha
			if(ReCaptcha::checkRecaptchaActive())
			{
				$count = 0;
				$contentReplace = preg_replace_callback(
					...
					function ($arImage)
					{
						...
						return ReCaptcha::callbackReplaceImage($arImage);
					},
					$content,
					-1,
					$count
				...
				$captcha_public_key = ReCaptcha::getPublicKey();
	...
	public static function onBeforeResultAddHandler($WEB_FORM_ID, &$arFields, &$arrVALUES){
		if(!defined('ADMIN_SECTION') && isset($_REQUEST['aspro_'.Solution::themesSolutionName.'_form_validate']))
		{

			foreach(GetModuleEvents(Solution::moduleID, "OnAsproBeforeResultAdd", true) as $arEvent)
				ExecuteModuleEventEx($arEvent, array($WEB_FORM_ID, &$arrVALUES));
	...
	public static function OnPageStartHandler()
	{
		...
		$captcha_public_key = ReCaptcha::getPublicKey();
		$captcha_version = ReCaptcha::getVersion();
		...
		{
			if($code = ReCaptcha::getCodeByPostList($arPostData))
			{
		...
		foreach($arPostData as $key => $arPost)
		{
			...
			if($code = ReCaptcha::getCodeByPostList($arPost))
			{
		...
		if($needReInit)
		{
			ReCaptcha::reInitContext($application, $request);
		}
	}
	...
	public static function onAfterResultAddHandler($WEB_FORM_ID, $RESULT_ID){
		Functions::addFormResultToIBlock($WEB_FORM_ID, $RESULT_ID);
		...
		if (CRM\Flowlu\Connection::getInstance(SITE_ID)->forms_autosend) {
			Functions::sendLeadCrmFromForm($WEB_FORM_ID, $RESULT_ID, 'FLOWLU', SITE_ID, false, false);
		}

		if (CRM\Amocrm\Connection::getInstance(SITE_ID)->forms_autosend) {
			Functions::sendLeadCrmFromForm($WEB_FORM_ID, $RESULT_ID, 'AMO_CRM', SITE_ID, false, false);
		}
	}
	...
	public static function OnAfterIBlockElementAddHandler($arFields){
		...
		if ($arFields['IBLOCK_ID'] != Cache::$arIBlocks[SITE_ID]["aspro_".Solution::themesSolutionName."_form"]["aspro_".Solution::themesSolutionName."_feedback"][0]) {
			return;
		}
		...
		Functions::sendDataToIBlock($arData);
	}

	public static function onAsproParametersHandler(&$arParams){
		$arNewOptions = Functions::getCustomBlocks();
		if ($arNewOptions) {
		...
		ThematicParameters::transformParameters($arParams);
	}
											...
											if ($matches) {
												...
												$arSubMenuItem = [
													...
													'LINK' => 'javascript: BX.ajax({
														url: \'/bitrix/admin/aspro.'.Solution::themesSolutionName.'_crm_acloud.php?SendCrm=Y&FORM_ID='.$formId.'&RESULT_ID='.$resultId.'&SITE_ID='.$siteId.'&sessid='.bitrix_sessid().'\',
														method: \'POST\',
														dataType: \'json\',
														async: false,
														start: true,

...
{
	const IBLOCK_TYPE_CONTENT = 'aspro_'.Solution::themesSolutionName.'_content';
	const IBLOCK_TYPE_CATALOG = 'aspro_'.Solution::themesSolutionName.'_catalog';
	...
	const RELATION_BLOCK_COMPLECTS = 'complects';
	...
	const ORDER_BLOCK_COMPLECTS = 'complects';
	...
	public static function hasCurrentValue($code)
	{
	...
	public static function getCurrentValues($code = '')
	{
		if ($code) {
			if (isset(self::$currentValues[$code]) && self::$currentValues[$code]) {
	...
	public static function appendTo(&$templateParameters)
	{
		...
		if (array_key_exists('USE_DETAIL_TABS', self::$baseParams)) {
			$moduleParamValue = Solution::GetFrontParametrValue(self::$useTabParam, self::getSiteId(), true);
	...
	{
		$pageBlocks = Solution::GetComponentTemplatePageBlocks(self::getAbsolutePath());
		$pageBlocksParams = Solution::GetComponentTemplatePageBlocksParams($pageBlocks);
		Solution::AddComponentTemplateModulePageBlocksParams(self::getAbsolutePath(), $pageBlocksParams);
		...
		if ($additionals) {
			foreach ($additionals as $param) {
				...
				Solution::AddComponentTemplateModulePageBlocksParams(self::getAbsolutePath(), $pageBlocksParams, $extParams, $listParams);
			}
		}

<?
namespace Aspro\Allcorp3Motor\Property\CustomFilter;

use \Bitrix\Main\Localization\Loc,
	\Bitrix\Main\Config\Option,
	Aspro\Allcorp3Motor\CrossSales,
	\CAllcorp3Motor as Solution,
	\Aspro\Allcorp3Motor\CGlobalCondCtrlComplex;
...
class CondCtrl extends CGlobalCondCtrlComplex{
	...
	public static function GetControlID(){
		return array('CondCrossIBField', 'CondCrossIBProp');
	}
	...
	public static function getLogicEvals($arLogics){
		$arAllLogics = array(
			'csEqual' => array(
				'ID' => 'csEqual',
				'VALUE' => 'csEqual',
				'LABEL' => Loc::getMessage('CUSTOM_FILTER_CONTROL_LOGIC_EQUAL')
			),
			'csNotEqual' => array(
				'ID' => 'csNotEqual',
				'VALUE' => 'csNotEqual',
				'LABEL' => Loc::getMessage('CUSTOM_FILTER_CONTROL_LOGIC_NOT_EQUAL')
			),
			'csIn' => array(
				'ID' => 'csIn',
				'VALUE' => 'csIn',
				'LABEL' => Loc::getMessage('CUSTOM_FILTER_CONTROL_LOGIC_INTERSECT')
			),
			'csNotIn' => array(
				'ID' => 'csNotIn',
				'VALUE' => 'csNotIn',
				'LABEL' => Loc::getMessage('CUSTOM_FILTER_CONTROL_LOGIC_NOT_INTERSECT')
			),
			'csSIn' => array(
				'ID' => 'csSIn',
				'VALUE' => 'csSIn',
				'LABEL' => Loc::getMessage('CUSTOM_FILTER_CONTROL_LOGIC_STRICT_INTERSECT')
			)
		);
		...
		if(
			...
			if(CrossSales::isCrossSalesIblock($propertyIblockId)){
				if(self::_isCrossSalesExtProductsFilterProperty($propertyCode)){
					$arResult[] = self::_addCrossSalesIblockFieldsControls($dataIblockId, $arParams);
			...
			if(CrossSales::isCrossSalesIblock($propertyIblockId)){
				if(self::_isCrossSalesExtProductsFilterProperty($propertyCode)){
					$arControlList = array_merge(
						$arControlList,
						self::_getCrossSalesIblockFieldsControls($dataIblockId, $strControlID),
    ...
    protected static function _addCrossSalesIblockFieldsControls($iblockId, $arParams){
    	$arResult = array(
			'controlgroup' => true,
			'group' =>  false,
			'label' => Loc::getMessage('CUSTOM_FILTER_CONTROL_CROSSALES_FIELD_GROUP'),
			'showIn' => static::GetShowIn($arParams['SHOW_IN_GROUPS']),
			'children' => array()
		);

		$arControls = self::_getCrossSalesIblockFieldsControls($iblockId);
    ...
    protected static function _getCrossSalesIblockFieldsControls($iblockId, $strControlID = false){
    	$arControlList = array();
    	...
    	foreach(
	    	...
	    	$fieldId = 'CondCrossIBField:'.$iblockId.':'.$field;
	    	$arControlList[$fieldId] = array(
				'ID' => $fieldId,
				'FIELD' => str_replace(':', '_', $fieldId),
				'FIELD_TYPE' => 'text',
				'LABEL' => $name,
				'CLASS_ID' => 'CondCrossIBField',
				'PREFIX' => Loc::getMessage('CUSTOM_FILTER_CONTROL_FIELD_PREFIX').' '.$name,
				'LOGIC' => static::getLogicEvals(array('csEqual', 'csNotEqual')),
				'MULTIPLE' => 'N',
				'PARENT' => true,
				'JS_VALUE' => array(
					'type' => 'lazySelect',
					...
					'load_params' => array(
						'action' => 'get_crosssales_iblockfields',
						'field' => $field,
						'iblockId' => $iblockId,
						'lang' => LANGUAGE_ID,
					),
